...
let endpoint= "http://ip-api.com/json/?fields=33612753" //583665";
...
L.tileLayer(
  "https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token={accessToken}",
  {
    ...
    zoomOffset: -1,
    accessToken:
      "pk.eyJ1IjoibGFzaGEzMTAiLCJhIjoiY2tmdHp1ZzRlMHBpMjJ5czhvZ21jeGdnaiJ9.iUlg-65qjkpL9P07zDlgpA",
  }
).addTo(mymap);
 ...
 const getIp = () => {
   fetch(endpoint)
     ...
     .then((response) => {
       mymap.flyTo([response.lat, response.lon], 13);
       L.marker([response.lat, response.lon]).addTo(mymap);
       ip.innerText = `${response.query}`;
       loc.innerText = `${response.city}, ${response.country}`;
       timezone.innerText = `UTC ${response.offset / 3600}:00`;//${response.timezone}
...
search.addEventListener("click", () => {
  let querr = document.getElementById("IP").value;
  endpoint = `http://ip-api.com/json/${querr}?fields=33612753`;
  ...
  if (expression.test(querr)) {
    // good IP
   getIp()
  } else {
    // bad IP
    alert("Please write valid IP");
  }
});